...
assign if_pc_next_default = if_pc_plus1;
...
// ID branch control logic (for BRZ/BRNZ)
...
always @(posedge clk or posedge reset) begin
    if(reset) begin
        ...
        // data signals
        ex_mem_rd_addr <= id_ex_rd_addr; // pass through rd address from ID/EX register
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        mem_wb_rd_addr <= ex_mem_rd_addr; // pass through rd address from EX/MEM register
