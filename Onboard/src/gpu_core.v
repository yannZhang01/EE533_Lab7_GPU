...
// ============================================================
// GPU Core: contains ONLY the original pipeline/control logic
// IMEM/DMEM are instantiated in gpu_top (wrapper)
// ============================================================
...
module gpu_core #(
    ...
    parameter OP_BRZ        = 4'b1011,
    parameter OP_BRNZ       = 4'b1100,
    parameter OP_MUL        = 4'b0110,
    parameter OP_TDOT       = 4'b1110,
    parameter OP_TDOT_RELU  = 4'b1111,

    parameter FREEZE_CLOCK  = 1
)(
    input  wire clk,
    input  wire reset,

    // -------- IMEM interface (from wrapper) --------
    output wire [INST_ADDR_WIDTH-1:0] if_imem_read_addr,
    input  wire [INST_WIDTH-1:0]      if_imem_instr,

    // -------- DMEM interface (to wrapper) --------
    output wire                       mem_dmem_read_en,
    output wire [DATA_ADDR_WIDTH-1:0] mem_dmem_read_addr,
    output wire [DATA_ADDR_WIDTH-1:0] mem_dmem_write_addr,
    output wire [DATA_WIDTH-1:0]      mem_dmem_write_data,
    output wire                       mem_dmem_write_en,
    input  wire [DATA_WIDTH-1:0]      mem_dmem_rdata
);
    ...
    assign if_imem_read_addr = if_pc_current;
    ...
    always @(posedge clk or posedge reset) begin
        if(reset) begin
            ...
            // data signals
            ex_mem_rd_addr <= ex_reg_write_addr;     // pass through rd address
            ex_mem_final_result <= ex_final_result;  // pass ALU/tensor result
            ex_mem_rs2_data <= id_ex_rs2_data;       // store data
        end
    end
    ...
    // ---- DMEM interface out to wrapper (preserve original semantics) ----
    assign mem_dmem_read_en     = mem_mread_en;
    assign mem_dmem_read_addr   = mem_final_result[DATA_ADDR_WIDTH-1:0];
    assign mem_dmem_write_addr  = mem_final_result[DATA_ADDR_WIDTH-1:0];
    assign mem_dmem_write_data  = mem_rs2_data;
    assign mem_dmem_write_en    = mem_mwrite_en;
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            mem_wb_rd_addr <= mem_reg_write_addr; // pass rd address
            mem_wb_alu_result <= mem_final_result; // pass ALU/tensor result
            mem_wb_dmem_rdata <= mem_dmem_rdata;   // pass memory read data
        end
    end
